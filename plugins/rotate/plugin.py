...
class Plugin:
    ...
    def start_config(self,frame,plug):
        self.exif_rotate=BooleanVar()
        self.exif_rotate.set(plug.settings.get('exif_rotate',0))
        Checkbutton(frame,text=_('Rotate from EXIF'),variable=self.exif_rotate).grid(row=0,column=0,padx=5,pady=5,sticky=W)


        self.need_rotate=BooleanVar()
        self.need_rotate.set(plug.settings.get('need_rotate',0))
        Checkbutton(frame,text=_('Rotate'),variable=self.need_rotate).grid(row=1,column=0,padx=5,pady=5,sticky=W)
        ...
        self.if_combo.grid(row=1,column=1,columnspan=1,padx=5,pady=5,sticky=W)
        ...
        self.rat_combo.grid(row=1,column=2,columnspan=1,padx=5,pady=5,sticky=W)
        self.rat_combo['values']=rts


        self.flip_h_var=BooleanVar()
        self.flip_h_var.set(plug.settings.get('flip_h',0))
        Checkbutton(frame,text=_('Flip horizontally'),variable=self.flip_h_var).grid(row=2,column=0,columnspan=3,padx=5,pady=5,sticky=W)



        self.flip_v_var=BooleanVar()
        self.flip_v_var.set(plug.settings.get('flip_v',0))
        Checkbutton(frame,text=_('Flip vertically'),variable=self.flip_v_var).grid(row=3,column=0,columnspan=3,padx=5,pady=5,sticky=W)
    ...
    def save_config(self):
        ...
        d['exif_rotate']=self.exif_rotate.get()
