...
class Plugin:
    ...
    def start_config(self,frame,plug):
        ...
        Checkbutton(frame, text=_("Do not open photos that have already been processed before."),variable=self.remember_var).grid(row=3,column=0,columnspan=2,padx=5,pady=5,sticky=W)

        self.auto_var=BooleanVar()
        self.auto_var.set(plug.settings.get('autoclean',1))
        Checkbutton(frame, text=_("Clear database at startup"),variable=self.auto_var,onvalue=1,offvalue=0).grid(row=4,column=0,columnspan=1,padx=5,pady=5,sticky=W)
        try:
        ...
        self.clean_but.grid(row=4,column=1,columnspan=1,padx=5,pady=5,sticky=W)

        self.watch_var=IntVar()
        self.watch_var.set(plug.settings.get('watch',0))
        Radiobutton(frame, text=_("Read the folder once"),variable=self.watch_var,value=0).grid(row=5,column=0,columnspan=2,padx=5,pady=5,sticky=W)
        Radiobutton(frame, text=_("Monitor the folder at intervals (sec)"),variable=self.watch_var,value=1).grid(row=6,column=0,columnspan=1,padx=5,pady=5,sticky=W) 


        self.secs_var=IntVar()
        self.secs_var.set(plug.settings.get('interval',5))
        Entry(frame,textvariable=self.secs_var,width=6).grid(row=6,column=1,padx=5,pady=5,sticky=W)
        ...
        rc.grid(row=7,column=0,columnspan=2,padx=5,pady=5,sticky=W)       
